...
<template>
  <div class="gruopSale">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>热销榜专区</el-breadcrumb-item>
    </el-breadcrumb>
    <h1>热销榜专区</h1>
    <div class="StatusRow">
    ...
    <div class="addbtn">
      <el-button type="primary" plain icon="el-icon-plus" @click="adddialog()">添加热销商品</el-button>
    </div>
    <!-- 添加拼团商品弹  -->
    <div class="dia">
    ...
    <div class="table">
      ...
      >
        ...
        <el-table-column align="center" prop="goods_sku_data" label="库存" width="90">
          <template slot-scope="{row}">
            <p v-for="(item, index) in row.goods_sku_data" :key="index">{{item.sku_stock}}</p>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    price() {
      ...
      if (arrs.length > 0) {
        arrs.forEach((item, i) => {
          this.$post("/backend/JdPromotion/hotGoodsAdd", {
            goods_id: item.goods_id,
            discount: item.rebate ? item.rebate : null
          }).then(res => {
            if (res.data.status == 1) {
              if (i == arrs.length - 1) {
    ...
    getTable() {
      this.loading = true;
      this.$get("/backend/JdPromotion/hotGoodsList", {
        search_goods_cate_id: this.option_val,
    ...
    //backend/JdPromotion/hotGoodsDelete热榜商品移除
    putaway(id) {
      this.$post("/backend/JdPromotion/hotGoodsDelete", {
        goods_ids: id
      }).then(res => {
        if (res.data.status == 1) {
          this.$message.success(res.data.msg);
    ...
    selectAll2(val) {
      ...
      this.str_id2 = this.rowIds.join(","); // 把数组转换成字符串
      // console.log(this.rowIds);
      return this.str_id2;

      // console.log(this.str_id2);
    },
    //页码操作
