<template>
  <div class="gruopSale">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>特卖专区</el-breadcrumb-item>
    </el-breadcrumb>
    <h1>特卖专区</h1>
    <div class="StatusRow">
    ...
    <div class="addbtn">
      <el-button type="primary" plain icon="el-icon-plus" @click="adddialog">添加特卖商品</el-button>
    </div>
    <!-- 添加商品弹窗  -->
    <div class="dia" v-if="dialogTableVisible">
      <el-dialog title :show-close="true" :visible.sync="dialogTableVisible" @close="clear">
        ...
        >
          ...
          <el-table-column align="center" label="设置折扣" width="150">
            <template slot-scope="{row}">
    ...
    <div class="table">
      ...
      >
        ...
        <el-table-column align="center" prop="goods_sku_data" label="原价(¥)" width="90">
          <template slot-scope="{row}">
            <p v-for="(item, index) in row.goods_sku_data" :key="index">{{item.sku_market_price}}</p>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" prop="goods_sku_data" label="特卖价（¥）" width="90">
          <template slot-scope="{row}">
            <p v-for="(item, index) in row.goods_sku_data" :key="index">{{item.sku_sale_price}}</p>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    price() {
      ...
      if (arrs.length > 0) {
        arrs.forEach((item, i) => {
          this.$post("/backend/JdPromotion/saleGoodsAdd", {
    ...
    dialogTable() {
      ...
      this.$get("/backend/JdPromotion/searchGoods", {
        ...
        console.log(this.dialogData, "table数据000000");
      });
    },

    // 查看
    goDetail(id) {
      this.$router.push({ path: "./specialDetail", query: { id: id } });
    },
    //弹窗input重置
    resetForm() {
    ...
    getTable() {
      this.loading = true;
      this.$get("/backend/JdPromotion/saleGoodsList", {
    ...
    ///特卖商品移除 backend/JdPromotion/saleGoodsDelete
    putaway(id) {
      // if (status == 0) {
      //   var ids = id;
      // }
      ...
      //   var ids = this.str_id;
      // }
      this.$post("/backend/JdPromotion/saleGoodsDelete ", {
        goods_ids: id
      }).then(res => {
        if (res.data.status == 1) {
          this.$message.success(res.data.msg);
