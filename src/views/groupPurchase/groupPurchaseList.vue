 ...
 * @LastEditors: Roc
 * @LastEditTime: 2020-03-20 12:24:08
 -->
<template>
...
export default {
  ...
  filters: {
    ...
    selectStatusFilter(status) {//订单状态
      const statusMap = {
        '-1': '已取消',
        1: '未支付',
        2: '待消费',
    ...
    actuallyPaid: function (row) {//实付金额
  ...
  methods: {
    ...
    toInfo(row) {
      ...
      this.$router.push({ name: 'groupPurchaseInfo', params: { row: row } })
    },
