<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>推荐专区（首页）</el-breadcrumb-item>
    </el-breadcrumb>
    <h1>首页广告审核列表</h1>
    <div style="display:flex">
      ...
      <div class="edit">
        <div class="title">
          <h2>编辑广告计费(外卖)</h2>
          <i class="el-icon-edit" @click="opendia(1)"></i>
        </div>
        <div class="list">
          <div>
      ...
      <div class="edit">
        <div class="title">
          <h2>编辑广告计费(商城)</h2>
          <i class="el-icon-edit" @click="opendia(2)"></i>
        </div>
        <div class="list">
          <div>
            <span style="margin-right:20px;">广告位数量：</span>
            <span>{{adv_num2}} 个</span>
          </div>
          <div>
            <span style="margin-right:38px;">按天计费：</span>
            <span>{{adv_price2}} 元</span>
          </div>
          <div>
            <span style="margin-right:38px;">套餐组合：</span>
            <span>{{adv_com2}}</span>
      ...
      <div class="edit">
        <div class="title">
          <h2>编辑广告计费(团购)</h2>
          <i class="el-icon-edit" @click="opendia(3)"></i>
        </div>
        <div class="list">
          <div>
            <span style="margin-right:20px;">广告位数量：</span>
            <span>{{adv_num3}} 个</span>
          </div>
          <div>
            <span style="margin-right:38px;">按天计费：</span>
            <span>{{adv_price3}} 元</span>
          </div>
          <div>
            <span style="margin-right:38px;">套餐组合：</span>
            <span>{{adv_com3}}</span>
...
export default {
  data() {
    return {
      ...
      num: "",
      price: "",
      system_off_desc: null,
      system_off_desc2: null
    };
  },
  watch: {
    shi() {
  ...
  methods: {
    ...
    getRule2() {
      this.$axios({
        ...
        // console.log(res.data.data, "商城");
        this.adv_num2 = res.data.data.daily_amount;
        this.adv_price2 = res.data.data.daily_price;
        this.adv_disconut2 = res.data.data.package_price;
        let arr = [];
        res.data.data.package_price.forEach(item => {
          arr.push(item.price + "元/" + item.day + "天");
        });
        this.adv_com2 = arr.join("、");
      });
    },
    getRule3() {
      this.$axios({
        ...
        // console.log(res.data.data, "团购");
        this.adv_num3 = res.data.data.daily_amount;
        this.adv_price3 = res.data.data.daily_price;
        this.adv_disconut3 = res.data.data.package_price;
        let arr = [];
        res.data.data.package_price.forEach(item => {
          arr.push(item.price + "元/" + item.day + "天");
        });
        this.adv_com3 = arr.join("、");
      });
    },
    advertList(page) {
      this.loading = true;
      this.$axios({
        method: "GET",
        url: "/backend/Advert/advertListfour",
        params: {
          module: "4",
    ...
    opendia(type) {
      ...
      if (type == 2) {
        this.setnum = this.adv_num2;
        this.setprice = this.adv_price2;
        if (this.adv_disconut2.length < 1) {
          this.discount = [{ day: "", price: "" }];
        } else {
          this.discount = this.adv_disconut2;
        }
      }
      if (type == 3) {
        this.setnum = this.adv_num3;
        this.setprice = this.adv_price3;
        if (this.adv_disconut3.length < 1) {
          this.discount = [{ day: "", price: "" }];
        } else {
          this.discount = this.adv_disconut3;
        }
      }
    },
    cancel() {
    ...
    open4(id) {
      ...
      this.$prompt("请输入驳回内容", "提示", {
        ...
        console.log(value);
        this.system_off_desc2 = value;
        if (value.trim()) {
          let str = value.trim();
          this.reject_advert(id, "/backend/Advert/rejectRefund", str);
        } else {
          this.$message.error("请输入拒绝内容");
        }
      });
    },
    //广告同意退款 /backend/Advert/advertRefund    广告拒绝退款 /backend/Advert/rejectRefund
