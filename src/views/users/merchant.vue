...
<template>
  <div class="commercial-wapper">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>二维码申请列表</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    todetail(id, ischange) {
      let arr = { shopid: id, ischange: ischange, erweima: 1 };
      sessionStorage.setItem("aaa", JSON.stringify(arr));
      sessionStorage.setItem("pages", this.page);
      this.$router.push({
        name: "merchantDetail",
        params: { shopid: id, change: ischange, erweima: 1 }
      });
    },
    //跳转到新增
    toAdd() {
    ...
    gettable(page) {
      ...
      this.$axios({
        method: "GET",
        url: "/backend/Merchant/merchantQrList",
        params: {
          id: this.shopid,
          contacts: this.name,
          mobile: this.mobile,
