...
<template>
  <div class="comdeetails-wepper">
    ...
    <section class="sdiv2">
      <span class="s1">审核状态</span>
      <el-radio-group v-model="radio2" @change="pass_refuse" v-show="status2==2">
        <el-radio :label="2">审核通过</el-radio>
        <el-radio :label="1">驳回申请</el-radio>
      </el-radio-group>
    ...
    <div class="re_input" v-if="ischanges ==1 && radio2==1">
    ...
    <div class="btn">
      <el-button
        ...
        v-show="status1==2||status2==2"
      >提 交</el-button>
      <el-button
        @click="xiu"
        type="primary"
        style="margin-right:20px;"
        v-show="status1==1&&status2==1"
      >修改权限</el-button>
...
export default {
  ...
  methods: {
    ...
    getdata() {
      this.$axios({
        ...
        if (res.data.data.shop_archive.archive_status == -1) {
          this.reject_reason2 = res.data.data.reject_reason;
        }
        if (
    ...
    pass_and_re(remark) {
      this.$axios({
        method: "POST",
        url: "/backend/Merchant/auditApply",
        data: {
          audit_seniority: this.radio2,
    ...
    onsubmit() {
      ...
      if (this.radio && this.radio2) {
        //一个驳回一个通过
        if (this.radio == 2 && this.radio2 == 1) {
          if (this.re_input2) {
            remark = this.re_input2;
          } else {
            this.$message.error("请填写驳回理由1");
          }
        }
        //一个驳回一个通过

        if (this.radio == 1 && this.radio2 == 2) {
          if (this.re_input) {
            remark = this.re_input;
          } else {
            this.$message.error("请填写驳回理由2");
          }
        }
        //同时驳回
        if (this.radio == 1 && this.radio2 == 1) {
          if (this.re_input2 && this.re_input) {
            remark = this.re_input + "#" + this.re_input2;
          } else {
            this.$message.error("请填写驳回理由3");
          }
        }
        //同时通过
        if (this.radio == 2 && this.radio2 == 2) {
          remark = "";
        }
        this.pass_and_re(remark);
      } else {
        // this.pass_and_re(remark);
        this.$message.error("必须一起审核");
      }
